...
def test_contract_is_frozen_and_unaltered():
    ...
    for relative_path, expected_hash in frozen_hashes.items():
        ...
        # 计算当前本地原始哈希，以及归一化换行符后的哈希，以支持跨平台换行符（CRLF/LF）一致性
        content = file_path.read_bytes()
        calculated_sha_raw = hashlib.sha256(content).hexdigest()
        calculated_hash_raw = f"sha256:{calculated_sha_raw}"

        normalized_content = content.replace(b"\r\n", b"\n")
        calculated_sha_lf = hashlib.sha256(normalized_content).hexdigest()
        calculated_hash_lf = f"sha256:{calculated_sha_lf}"
        
        # 只要有一项能匹配预期哈希，即视为契约未篡改
        assert expected_hash in (calculated_hash_raw, calculated_hash_lf), (
            f"Silent modification detected! Schema '{relative_path}' has been edited after contract freeze.\n"
            f"Expected: {expected_hash}\n"
            f"Calculated (Raw): {calculated_hash_raw}\n"
            f"Calculated (LF Normalized): {calculated_hash_lf}\n"
